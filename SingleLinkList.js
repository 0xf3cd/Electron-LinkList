...
/*****************************************************
					默认常量及工具函数
******************************************************/
const SINGLE_LINKLIST_LENGTH = 10;
...
/**
 * 结点类，表示链表中某一个结点
 * 不在这个类的方法中进行输入正确性判断, 在 SingleLinkList 类中进行判断
 * @class
 */
class Node {
	/**
...
/**
 * createSingleLinkList 方法: 创建一个单向链表
 * @public
 * @param {int=} length: 可选参数, 链表的长度, 默认为 SINGLE_LINKLIST_LENGTH
 * @param {bool=} random: 可选参数, 是否随机生成, 为 true 表示随机生成, 默认为 true
 * @param {array=} values: 可选参数, 若不随机生成, 则需要以数组形式输入各结点的值
 * @return {void}
 */
SingleLinkList.prototype.createSingleLinkList = function(length, random, values) {
	if(this._length !== 0) {
		return;//如果链表非空, 则不需要创建链表
	}

	if(MyTools.isInteger(length) !== true) {
		length = SINGLE_LINKLIST_LENGTH;//如果参数 length 不为整数, 则重置为默认值
	}
	this._length = length;
...
/**
 * deleteElement 方法: 删除单链表中的一个结点
 * @public
 * @param {int} index: 删除第 index 个结点
 * @return {void}
 */
SingleLinkList.prototype.deleteElement = function(index) {
	if(MyTools.isInteger(index) !== true) {
 ...
 */
SingleLinkList.prototype.findElement = function(value) {
	if(MyTools.isInteger(value) !== true) {
 ...
 * @return {int}: 该结点的值
 */
SingleLinkList.prototype.getElementValue = function(index) {
	if(MyTools.isInteger(index) !== true) {
		return;//如果 index 为非整数则结束
	}
 ...
 * @return {void}
 */
SingleLinkList.prototype.setElementValue = function(index, value) {
	if(MyTools.isInteger(index) !== true) {
		return;//如果 index 为非整数则结束
	}
...
/**
 * destroySingleLinkList 方法: 销毁链表
 * @public
 * @return {void}
 */
SingleLinkList.prototype.destroySingleLinkList = function() {
	if(this._length === 0) {
		return;//为空则不需要退出
	}
 ...
 * @return {void}
 */
SingleLinkList.prototype.showAllElementsInConsole = function() {
	let current = this._head.getNext();
	while(current !== null) {
 ...
 */
SingleLinkList.prototype.getAllElements = function() {
	let current = this._head.getNext();
 ...
 * @return {int}: 结点数
 */
SingleLinkList.prototype.getLength = function() {
	return this._length;
};
